				...
				if (str.StartsWith("Circle(") || str.StartsWith("CIRCLE("))
				{
					...
					if (idx > 0)
					{
						...
						if (idx > 0)
						{
							...
							{
								if (!Double.TryParse(arg.Substring(idx + 1), NumberStyles.Number, CultureInfo.InvariantCulture, out d))
						...
						{
							if (!Double.TryParse(arg, NumberStyles.Number, CultureInfo.InvariantCulture, out d)) throw new InvalidShapeException("Missing Distance: " + str);
						}
